...
const SeatChart = ({ occasion, cryptoSeats, provider, setToggle }) => {
  const [seatsTaken, setSeatsTaken] = useState(false)
  const [hasSold, setHasSold] = useState(false)

  const getSeatsTaken = async () => {
    const seatsTaken = await cryptoSeats.getSeatsTaken(occasion.id)
  ...
  if (balance.lt(occasion.cost)) {
    ...
    alert("Insufficient funds to buy the ticket.");
    return;
  }
    const transaction = await cryptoSeats.connect(signer).mint(occasion.id, _seat, { value: occasion.cost })
